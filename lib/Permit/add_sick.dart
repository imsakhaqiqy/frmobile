...
class AddSickScreen extends StatefulWidget {
  const AddSickScreen({Key? key}) : super(key: key);

  State<AddSickScreen> createState() => AddSickScreenState();
}

class AddSickScreenState extends State<AddSickScreen> {
  ...
  Future _saveSick() async {
    ...
    var url =
        'https://www.nabasa.co.id/api_field_recruitment_v1/services.php/saveSick';
    ...
    if (response.statusCode == 200) {
      var message = jsonDecode(response.body)['Save_Sick'];
      if (message == 'Save Success') {
        Alert(
          context: context,
          style: alertStyle(),
          type: AlertType.success,
          desc: "Tambah Sakit berhasil",
          buttons: [
            DialogButton(
              child: Text(
                "OK",
                        ...
                        if (formKey.currentState!.validate()) {
                          if (imageFilex == null) {
                            _scaffoldKey.currentState?.showSnackBar(SnackBar(
                              content: Text('Ambil foto surat sakit dulu ya'),
